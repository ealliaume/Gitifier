...
static NSString *bzrExecutable = nil;

@implementation Bzr

@synthesize repositoryUrl;

+ (NSString *) bzrExecutable {
  return bzrExecutable;
}

+ (void) setBzrExecutable: (NSString *) path {
  [self willChangeValueForKey: @"bzrExecutable"];
  path = [path psTrimmedString];
  bzrExecutable = [path isEqual: @""] ? nil : path;
  [self didChangeValueForKey: @"bzrExecutable"];
  PSNotifyWithData(BzrExecutableSetNotification, PSHash(@"path", path));
}
...
- (void) runCommand: (NSString *) command withArguments: (NSArray *) arguments inPath: (NSString *) path {
  ...
  if (!bzrExecutable) {
    [self notifyDelegateWithSelector: @selector(commandFailed:output:)
                             command: command
                              output: @"No Bzr executable found."];
    return;
  }

  currentData = [[NSMutableData alloc] init];

  PSLog(@"running command bzr %@ %@", command, arguments);
  ...
  currentTask.launchPath = bzrExecutable;
  ...
  if (repositoryUrl) {
    ...
    NSDictionary *customEnvironment = [[NSUserDefaults standardUserDefaults] objectForKey: @"bzrEnvironment"];
...
- (void) executeTask {
  @try {
    ...
    if (cancelled) {
      ...
      if (status == 0) {
        PSLog(@"command bzr %@ completed with output: %@", command, output);
        ...
        PSLog(@"command bzr %@ failed with output: %@", command, output);
    ...
    currentTask = nil;
    currentData = nil;
    PSLog(@"command bzr %@ failed with exception: %@", command, e);
